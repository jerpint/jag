...
def main(argv):
    # Builds, trains and evaluates a tf.estimator.
    # Then, exports it for inference, logs the exported model
    ...
    feat_cols = [
        tf.feature_column.numeric_column(key="features", shape=(x_train.shape[1],))]
    feat_spec = {
    ...
    train_input_fn = tf.estimator.inputs.numpy_input_fn({"features": x_train},
                                                        y_train,
                                                        num_epochs=None,
                                                        shuffle=True)
    with mlflow.start_run() as tracked_run: # noqa
        mlflow.log_param("Hidden Units", hidden_units)
        mlflow.log_param("Steps", steps)
        regressor.train(train_input_fn, steps=steps)
        test_input_fn = tf.estimator.inputs.numpy_input_fn({"features": x_test},
                                                           y_test,
                                                           num_epochs=None,
                                                           shuffle=True)
        ...
        try:
            saved_estimator_path = (
                regressor.export_savedmodel(temp, receiver_fn).decode("utf-8"))
